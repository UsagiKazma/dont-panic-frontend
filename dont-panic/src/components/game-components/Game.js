...
export default function Game({ wordsArr, handleScore, handleLost }) {
  ...
  if (wordsArr && wordsArr.length) {
    ...
    let random = "";
    //difficulty filtered
    let easy = wordsArr.filter((diff) => {
      if (diff && diff.difficulty === "easy") {
        return diff.word;
      } else return blank;
    });
    let medium = wordsArr.filter((diff) => {
      if (diff && diff.difficulty === "medium") {
        return diff.word;
      } else return blank;
    });
    let hard = wordsArr.filter((diff) => {
      if (diff && diff.difficulty === "hard") {
        return diff.word;
      } else return blank;
    });
    let dangerous = wordsArr.filter((diff) => {
      if (diff && diff.difficulty === "dangerous") {
        return diff.word;
      } else return blank;
    });
    ...
    function handleDifficulty() {
      if (level <= 3) {
        random = generate(easy);
      } else if (level >= 4) {
        random = generate(medium);
      } else if (level >= 6) {
        random = generate(hard);
      } else if (level >= 9) {
        random = generate(dangerous);
      } else {
        random = generate(wordsArr);
      }
    }
    handleDifficulty();
